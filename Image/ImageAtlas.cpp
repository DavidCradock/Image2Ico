...
namespace X
{
	...
	{
		ThrowIfTrue(0 == vstrImages.size(), "The passed vector<string> containing individual image filenames contains no data.");
		ThrowIfTrue(0 == uiMaxAtlasImageWidth || 0 == uiMaxAtlasImageHeight, "uiMaxAtlasImageWidth or uiMaxAtlasImageHeight was set to zero.");
		...
		for (unsigned int ui = 0; ui < vstrImages.size(); ui++)
		{
			...
			ThrowIfFalse(pNewImage, "Unable to allocate memory for individual image.");
			vImages.push_back(pNewImage);

			// Load each individual image
			ThrowIfFalse(vImages[ui]->load(vStrImageFilenames[ui], false), "Unable to load image from file: " + vStrImageFilenames[ui] + ".");
...
//			if (bNotSameDims)
//			{
//				std::string strTxt = "All images added to an atlas must have the same dimensions.";
...
//			}
		...
		ThrowIfTrue(uiMaxImageWidth > uiMaxAtlasImageWidth, "An individual image's width(" + std::to_string(uiMaxImageWidth) + "), including spacing of " + std::to_string(uiImageSpacing) + " is greater than the specified max atlas image's width(" + std::to_string(uiMaxAtlasImageWidth) + ".");
		ThrowIfTrue(uiMaxImageHeight > uiMaxAtlasImageHeight, "An individual image's height(" + std::to_string(uiMaxImageHeight) + "), including spacing of " + std::to_string(uiImageSpacing) + " is greater than the specified max atlas image's height(" + std::to_string(uiMaxAtlasImageHeight) + ".");
		...
		{
			CImage* pNewImage = new CImage;
			ThrowIfFalse(pNewImage, "Unable to allocate memory for atlas image.");
		...
		for (unsigned int ui = 0; ui < vImages.size(); ui++)
		{
			...
			ThrowIfTrue(vAtlasDims.x < 1.0f, "Atlas width less than 1.");
			ThrowIfTrue(vAtlasDims.y < 1.0f, "Atlas height less than 1.");
	...
	{
		ThrowIfTrue(0 == _mvAtlasImages.size(), "There are no atlas images.");
		ThrowIfTrue(uiIndex >= _mvAtlasImages.size(), "Invalid atlas index given.");
		return _mvAtlasImages[uiIndex];
	}

	std::vector<CImageAtlasDetails> CImageAtlasPacker::getAllImageDetails(void) const
	{
		ThrowIfFalse(_mvImageDetails.size(), "No image data currently exists.");
	...
	CImageAtlasDetails CImageAtlasPacker::getImageDetails(const std::string& strImageName) const
	{
		...
		ThrowIfTrue(it == _mmapImageDetails.end(), "Named image doesn't exist.");
		return it->second;
	}
