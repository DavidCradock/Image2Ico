...
namespace X
{
	CProfiler::CProfiler()
	{
		LOG("CProfiler constructor called.");
		reset();
	}
	...
	void CProfiler::end(const std::string& strSectionName)
	{
		...
		auto it = _mmapSections.find(strSectionName);
		ThrowIfTrue(_mmapSections.end() == it, "The named section doesn't exist. (\"" + strSectionName + "\")");
	...
	std::vector<SProfilerResults> CProfiler::getResults(bool bReturnCachedResults)
	{
		...
		ThrowIfTrue(_mmapSections.end() == itMain, "The \"main\" section doesn't exist.");
