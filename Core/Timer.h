...
namespace X
{
    ...
    public:
        ...
        /// \brief Returns the minimum framerate achieved so far
        /// 
        /// \return The minimum framerate achieved so far
        /// 
        /// There is a delay of 1 second before this value is calculated after initial construction or a call to reset()
        float getFPSMinimum(void) const;

        /// \brief Returns the maximum framerate achieved so far
        /// 
        /// \return The maximum framerate achieved so far
        /// 
        /// There is a delay of 1 second before this value is calculated after initial construction or a call to reset()
        float getFPSMaximum(void) const;
    ...
    private:
        ...
        double _mdFPSMinimum;                   ///< Minimum achieved frame rate so far
        double _mdFPSMaximum;                   ///< Maximum achieved frame rate so far
        double _mdFPSMinMaxDelay;               ///< Delay from start until these values are calculated
        bool _mbFPSMinMaxInit;                  ///< Used to initialise min/max FPS values once the initial delay has passed.
    };
}